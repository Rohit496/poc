...
describe("buildSlices", () => {
  ...
  it("accepts a custom DonutConfig and produces valid paths", () => {
    const totals = { ...ALL_ZERO_TOTALS, Food: 100 };
    const customConfig = { outerRadius: 50, innerRadius: 20, center: 60 };
    const result = buildSlices(totals, CATEGORY_COLORS, customConfig);
    expect(result).toHaveLength(1);
    expect(result[0].path.length).toBeGreaterThan(0);
    expect(result[0].path).not.toContain("NaN");
  });
