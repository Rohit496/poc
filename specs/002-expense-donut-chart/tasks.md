...
- [x] T001 Create `apps/expense-tracker/src/app/utils/` directory (add `.gitkeep` placeholder; will be replaced by T005)
- [x] T002 [P] Add `CategorySlice` interface and `CategoryView` type to `apps/expense-tracker/src/app/models/expense.model.ts` — `CategorySlice: { category, amount, percentage, color, path, startAngle, sweepAngle }`, `CategoryView: 'list' | 'chart'`
- [x] T003 [P] Add `CATEGORY_COLORS`, `CATEGORY_VIEW_STORAGE_KEY`, `DONUT_OUTER_RADIUS` (80), `DONUT_INNER_RADIUS` (45), `DONUT_CENTER` (100) constants to `apps/expense-tracker/src/app/constants/categories.ts`
